...
import { KEY_DATA_FETCHED, KEY_PUBLIC_KEY } from '@/core/constants/store';
import { useSession } from '@/core/context/SessionProvider';
import {
  configureMessageDate,
  createMessageReport,
} from '@/core/functions/chat';
import {
...
import {
  ...
  decryptMessages,
  encryptMessage,
} from '@/core/functions/encryption';
...
const ChatProvider = ({ children }: PropsWithChildren) => {
  ...
  const acceptInvitation = async (roomCreator: UserItem): Promise<boolean> => {
    ...
    try {
      ...
      const sharedKeySuccess = await createSharedKey(
        roomCreatorId,
  ...
  const fetchChatData = async () => {
    ...
    // console.log('fetchChatData: data', data);
    const {
      roomItems,
      roomMembers,
    ...
    try {
      ...
      const curUserSharedKeySuccess = createSharedKey(
        userId,
      ...
      for (let m of roomMembers) {
        const success = createSharedKey(m.id, m.publicKey);
        if (!success) return;
      }
      // console.info(`[ F ] INI3 Shared keys created / initChatData`);

      // Decrypt messages
      const messages = await decryptMessages(encryptedMessages);

      // Add new messages to the map
      const updNewMsgMap = addNewMessagesToMap(messages, false);
      if (updNewMsgMap) {
  ...
  const initRooms = async () => {
    ...
    for (let m of newRoomMembers) {
      ...
      const sharedKeySuccess = await createSharedKey(m.id, m.publicKey);
  ...
  const initMessages = async () => {
    ...
    const res = await getNewMessages({ userId, token });
    if (!res?.data) {
      const errMsg =
    ...
    const newMessages = await decryptMessages(encryptedNewMessages);

    // Add new messages to the map
    const updNewMsgMap = addNewMessagesToMap(newMessages, true);
    if (updNewMsgMap) {
      updateNewMsgMap(updNewMsgMap);
      console.info(`[ F ] CHP3 New message map updated / initNewMessages`);

      // Add messages to the 'message' table
      const addToDb = await saveMessagesInLocalDb(newMessages, memberMap);
      if (addToDb) {
        console.info(
  ...
  const onMessage = async (
    ...
    try {
      const decryptedMessages = await decryptMessages([encryptedMessage]);
      if (!decryptedMessages.length) {
        logMessage(`[ CHP ] Unable to decrypt message`, 'error');
        return;
      }
      message = decryptedMessages[0];
      logMessage(`[ CHP ] New message decrypded`);
    } catch (err: any) {
      console.error(err);
      return;
    }
